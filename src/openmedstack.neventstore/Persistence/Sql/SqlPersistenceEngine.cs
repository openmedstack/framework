namespace OpenMedStack.NEventStore.Persistence.Sql
{
    ...
    public class SqlPersistenceEngine : IPersistStreams
    {
        ...
        public virtual async Task Initialize()
        {
            ...
            Logger.Debug(PersistenceMessages.InitializingStorage);
        ...
        {
            Logger.Debug(PersistenceMessages.GettingAllCommitsBetween, streamId, minRevision, maxRevision);
         ...
         CancellationToken cancellationToken = default)
        {
            ...
            Logger.Debug(PersistenceMessages.GettingAllCommitsFrom, start, bucketId);
            ...
            CancellationToken cancellationToken)
        {
            ...
            Logger.Debug(PersistenceMessages.GettingAllCommitsFromTo, start, end);
        ...
        public virtual async Task<ICommit?> Commit(CommitAttempt attempt)
        {
            ...
            try
            {
                ...
                Logger.Debug(PersistenceMessages.CommitPersisted, attempt.CommitId);
            }
            catch (Exception e)
            {
                ...
                {
                    Logger.Info(PersistenceMessages.DuplicateCommit);
                    throw new DuplicateCommitException(e.Message, e);
                }

                Logger.Info(PersistenceMessages.ConcurrentWriteDetected);
        ...
        public virtual IAsyncEnumerable<IStreamHead> GetStreamsToSnapshot(string bucketId, int maxThreshold, CancellationToken cancellationToken)
        {
            Logger.Debug(PersistenceMessages.GettingStreamsToSnapshot);

            async IAsyncEnumerable<IStreamHead> Query(IDbStatement query, [EnumeratorCancellation] CancellationToken token)
            {
        ...
        public virtual Task<ISnapshot?> GetSnapshot(
            ...
            CancellationToken cancellationToken)
        {
            Logger.Debug(PersistenceMessages.GettingRevision, streamId, maxRevision);
            var streamIdHash = _streamIdHasher.GetHash(streamId);

            async IAsyncEnumerable<ISnapshot> Query(IDbStatement query, [EnumeratorCancellation] CancellationToken token)
            {
        ...
        {
            Logger.Debug(PersistenceMessages.AddingSnapshot, snapshot.StreamId, snapshot.StreamRevision);
        ...
        {
            Logger.Warn(PersistenceMessages.PurgingStorage);
            return ExecuteCommand(cmd => cmd.ExecuteNonQuery(_dialect.PurgeStorage));
        }

        public Task Purge(string bucketId)
        {
            Logger.Warn(PersistenceMessages.PurgingBucket, bucketId);
        ...
        public IAsyncEnumerable<ICommit> GetFrom(
            ...
            CancellationToken cancellationToken)
        {
            Logger.Debug(PersistenceMessages.GettingAllCommitsFromBucketAndCheckpoint, bucketId, checkpointToken);
        ...
        {
            Logger.Debug(PersistenceMessages.GettingAllCommitsFromCheckpoint, checkpointToken);
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            Logger.Debug(PersistenceMessages.ShuttingDownPersistence);
            IsDisposed = true;
        }
        ...
        protected virtual IAsyncEnumerable<T> ExecuteQuery<T>(Func<IDbStatement, CancellationToken, IAsyncEnumerable<T>> query, CancellationToken cancellationToken)
        {
            ThrowWhenDisposed();
            ...
            try
            {
                ...
                Logger.Verbose(PersistenceMessages.ExecutingQuery);
            ...
            catch (Exception e)
            {
                statement.Dispose();
                connection.Dispose();
                Logger.Debug(PersistenceMessages.StorageThrewException, e.GetType());
        ...
        private void ThrowWhenDisposed()
        {
            ...
            Logger.Warn(PersistenceMessages.AlreadyDisposed);
        ...
        protected virtual async Task<T> ExecuteCommand<T>(Func<IDbConnection, IDbStatement, Task<T>> command)
        {
            ...
            try
            {
                Logger.Verbose(PersistenceMessages.ExecutingCommand);
                var rowsAffected = await command(connection, statement).ConfigureAwait(false);
                Logger.Verbose(PersistenceMessages.CommandExecuted, rowsAffected);
            ...
            catch (Exception e)
            {
                ...
                Logger.Info(PersistenceMessages.RecoverableExceptionCompletesScope);

                throw;
            }
        }

        private static bool RecoverableException(Exception e) => e is UniqueKeyViolationException || e is StorageUnavailableException;
