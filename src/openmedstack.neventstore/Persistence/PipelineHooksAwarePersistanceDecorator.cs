...
namespace OpenMedStack.NEventStore.Persistence
{
    ...
    public class PipelineHooksAwarePersistanceDecorator : IPersistStreams
    {
        ...
        public IAsyncEnumerable<ICommit> GetFrom(string bucketId, string streamId, int minRevision, int maxRevision, CancellationToken cancellationToken = default)
        {
        ...
        public Task<ISnapshot?> GetSnapshot(
            ...
            string streamId,
            int maxRevision,
            CancellationToken cancellationToken) => _original.GetSnapshot(bucketId, streamId, maxRevision, cancellationToken);

        public Task<bool> AddSnapshot(ISnapshot snapshot) => _original.AddSnapshot(snapshot);

        public IAsyncEnumerable<IStreamHead> GetStreamsToSnapshot(string bucketId, int maxThreshold, CancellationToken cancellationToken) => _original.GetStreamsToSnapshot(bucketId, maxThreshold, cancellationToken);
        ...
        private async IAsyncEnumerable<ICommit> ExecuteHooks(IAsyncEnumerable<ICommit> commits, [EnumeratorCancellation] CancellationToken cancellationToken)
        {
