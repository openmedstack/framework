namespace OpenMedStack.NEventStore.Client
{
    ...
    public class PollingClient2 : IDisposable
    {
        ...
        private static readonly TaskStatus[] disposable = new[] { TaskStatus.RanToCompletion, TaskStatus.Faulted, TaskStatus.Canceled };
        ...
        public PollingClient2(IPersistStreams persistStreams, Func<ICommit, Task<HandlingResult>> callback, TimeSpan waitInterval = default)
        {
        ...
        private async Task<bool> InnerPoll()
        {
            ...
            if (Interlocked.CompareExchange(ref _isPolling, 1, 0) == 0)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    _logger.Error($"Error during polling client {ex}");
                }
        ...
        public void Dispose(bool isDisposing)
        {
            ...
            if (isDisposing)
            {
                ...
                if (_pollingThread != null && disposable.Contains(_pollingThread.Status))
                {
